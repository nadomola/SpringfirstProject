...
@Getter // 각 필드값을 조횧할 수 있는 getter 메서드 자동 생성
...
public class Comment {
    ...
    public static Comment createComment(CommentDto dto, Article article) {
        // 예외 발생
        if ( dto.getId() != null)
            throw new IllegalArgumentException("댓글 생성 싶패 ! 댓글의 id가 없어야 합니다.");
        if (dto.getArticleId() != article.getId())
            throw new IllegalArgumentException("댓글 생성 실패! 게시글의 id가 잘못 되었습니다.");
        // 엔티티 생성 및 반환 (예외 없다면 엔티티 만들어 반환)
        return new Comment(
                dto.getId(),  // 댓글 아이디
                article,   // 부모 게시글
